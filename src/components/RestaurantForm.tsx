...
const RestaurantForm: React.FC<RestaurantFormProps> = ({ onSubmit, initialData }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const restaurantData = { name, tel, address, postalCode, province, foodtype, picture };

    try {
      const method = initialData ? 'PUT' : 'POST'; // Use PUT for editing, POST for creating
      const url = initialData ? `/api/restaurants/${initialData._id}` : '/api/restaurants';
      ...
      if (response.ok) {
        alert(initialData ? 'Restaurant updated successfully!' : 'Restaurant created successfully!');
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-sm flex flex-col gap-2 justify-center">
